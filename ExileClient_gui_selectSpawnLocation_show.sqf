...
/*
	...
	or added
*/
if!(DTG_BaseMarkers isEqualTo []) then
{
	{
		deleteMarkerLocal _x;
	} forEach DTG_BaseMarkers;
	DTG_BaseMarkers = [];
};
{
	...
	if(_uid in _flag) then 
	{
		...
		DTG_BaseMarkers pushBack _marker;
	};
...
_numberOfSpawnPoints = 0;
{
	...
	{
		if!(_x in DTG_LastSpawnLocation) then
		{
